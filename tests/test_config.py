...
def test_load_config_uses_jobpipe_config_overlay_env(tmp_path: Path, monkeypatch) -> None:
    base = tmp_path / "base.yaml"
    overlay = tmp_path / "overlay.yaml"
...
def test_explicit_overlay_is_applied_after_env_overlay(tmp_path: Path, monkeypatch) -> None:
    base = tmp_path / "base.yaml"
    ...
    monkeypatch.setenv(
        "JOBPIPE_CONFIG_OVERLAY",
        os.pathsep.join([str(env_overlay)]),
    )
